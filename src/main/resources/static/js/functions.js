              ...
              connectWS: function() {
                ...
                if (currApp.webSocket !== null && currApp.webSocket.readyState !== WebSocket.CLOSED) {
                    return;
                      ...
                      // Create a websocket
                currApp.webSocket = new WebSocket("ws://localhost:9999/job-progress-web-socket");
                currApp.webSocket.onopen = function(event) {
                    currApp.showLink = true;
                };
                currApp.webSocket.onmessage = function(event) {
                    currApp.jsonParse = JSON.parse(event.data);
                    currApp.percentProgress = currApp.jsonParse.percent.percent.toFixed(2) + '%';
                    currApp.lastJobLink = currApp.jsonParse.name;
                    if(currApp.percentProgress === '100%') {
                        clearInterval(intervalCheck);
                        window.sessionStorage.removeItem('showProcess');
                    }
